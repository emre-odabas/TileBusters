...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackSound : MMFeedback
    {
        ...
        public float FadeDurationOnStop;
        ...
        {
            if (Active && SettingsManager.Instance.m_IsSoundOn)
            {
                if (Sfx != null)
                {
        ...
        protected override void CustomStopFeedback(Vector3 position, float attenuation = 1.0f)
        {
            if (Active && SettingsManager.Instance.m_IsSoundOn)
            {
                ...
                if (RandomSfx.Length > 0)
                {
                    ...
                    if (_randomClip != null)
                    {
                        StopSound(_randomClip, position);
                    }
        ...
        protected virtual void StopSound(AudioClip sfx, Vector3 position)
        {
            ...
            {
                // we set that audio source clip to the one in paramaters
                StopAudioSource(_cachedAudioSource);
            }
            ...
            if (PlayMethod == PlayMethods.Pool)
            {
                ...
                if (_tempAudioSource != null)
                {
                    StopAudioSource(_tempAudioSource);
                }
            }
        }
